...
export function approximateSpline(spline, steps = 100) {
  ...
  return points
    .slice(0, -1)
    .map((p, i) => ({ startP: p, endP: points[i + 1] }));
}
...
export function bezierClippingIntersect(
  bezier1,
  bezier2,
  depth = 0,
  maxDepth = 15,
  ...
  if (
    depth >= maxDepth ||
    (Math.max(box1.maxX - box1.minX, box1.maxY - box1.minY) < tolerance &&
      Math.max(box2.maxX - box2.minX, box2.maxY - box2.minY) < tolerance)
  ) {
    // Return approximate intersection point at mid of bounding boxes
    const ix =
      (Math.max(box1.minX, box2.minX) + Math.min(box1.maxX, box2.maxX)) / 2;
    const iy =
      (Math.max(box1.minY, box2.minY) + Math.min(box1.maxY, box2.maxY)) / 2;
    return [{ x: ix, y: iy }];
  }
...
export function intersectLineQuadBezier(line, bezier) {
  ...
  const A =
    a * (bezP0.x - 2 * bezP1.x + bezP2.x) +
    b * (bezP0.y - 2 * bezP1.y + bezP2.y);
...
export function intersectLineEllipseArc(line, ellipse) {
  ...
  const b =
    2 * ((p1Shifted.x * dx) / (rx * rx) + (p1Shifted.y * dy) / (ry * ry));
  const c =
    (p1Shifted.x * p1Shifted.x) / (rx * rx) +
    (p1Shifted.y * p1Shifted.y) / (ry * ry) -
    1;
  ...
  if (discriminant === 0) {
    ...
    const t = -b / (2 * a);
    if (t >= 0 && t <= 1) {
      // Chỉ lấy giao điểm nằm trên đoạn thẳng
      intersections.push({
        x: p1.x + t * dx,
        y: p1.y + t * dy,
      });
  ...
  return intersections.filter((p) => isPointOnEllipseArc(p, ellipse));
}
...
export function intersectEllipticalArcArc(ellipArc, arc, sampleStep = 0.01) {
  ...
  return circlePoints.filter((point) => {
    return isPointOnArc(point, arc) && isPointOnEllipseArc(point, arc);
  });
}
...
function findCircleEllipseIntersection(circleCenter, r, ellipseCenter, rx, ry) {
  ...
  for (let x = xMin; x <= xMax; x += step) {
    ...
    const ellipseRadical =
      1 - (xFromEllipseCenter * xFromEllipseCenter) / (rx * rx);
        ...
        for (const yEllipse of yValuesFromEllipse) {
          for (const yCircle of yValuesFromCircle) {
            ...
            if (Math.abs(yEllipse - yCircle) < step * 2) {
              ...
              const isTooClose = intersections.some(
                (point) =>
                  Math.abs(point.x - x) < step * 2 &&
                  Math.abs(point.y - yEllipse) < step * 2
              );
  ...
  return refineIntersectionPoints(
    intersections,
    circleCenter,
    r,
    ellipseCenter,
 ...
 */
function refineIntersectionPoints(
  points,
  circleCenter,
  r,
  ellipseCenter,
...
export function intersectEllipseArcEllipseArc(arc1, arc2, samples = 1000) {
  const result = [];
  const { centerP: c1, rx: rx1, ry: ry1, startP: sp1, endP: ep1 } = arc1;

  // Tính góc bắt đầu & kết thúc cho arc1
  const angleStart1 = getAngleOnEllipse(sp1, c1.x, c1.y, rx1, ry1);
  const angleEnd1 = getAngleOnEllipse(ep1, c1.x, c1.y, rx1, ry1);

  // Tính hướng: clockwise hay counterclockwise
  let delta = angleEnd1 - angleStart1;
  if (delta < 0) delta += 2 * Math.PI;
  const isClockwise = delta > Math.PI;

  for (let i = 0; i <= samples; i++) {
    const t = i / samples;
    const angle = angleStart1 + (isClockwise ? -1 : 1) * delta * t;
    const x = c1.x + rx1 * Math.cos(angle);
    const y = c1.y + ry1 * Math.sin(angle);
    const pt = { x, y };

    if (isPointOnEllipseArc(pt, arc2)) {
      // Tránh trùng lặp
      const last = result[result.length - 1];
      if (!last || Math.hypot(last.x - x, last.y - y) > 1e-3) {
        result.push(pt);
