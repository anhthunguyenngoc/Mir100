...
export function aStar(start, goal, obstacles, mapWidth, mapHeight) {
  ...
  while (!openList.isEmpty()) {
    ...
    for (let neighbor of neighbors) {
      ...
      if (
        neighborPoint.x < 0 ||
        neighborPoint.x >= mapWidth ||
        neighborPoint.y < 0 ||
        neighborPoint.y >= mapHeight ||
      ...
      const tentativeGScore =
        ...
        calculateManhattanDistance(current, neighborPoint);

      // Nếu điểm chưa được thêm vào openList hoặc gScore thấp hơn
      if (
        !openList.queue.some(
          (item) =>
            item.item.x === neighborPoint.x && item.item.y === neighborPoint.y
        )
      ) {
        openList.enqueue(
          neighborPoint,
          tentativeGScore + heuristic(neighborPoint, goal)
        );
      } else if (
        tentativeGScore >= gScore[`${neighborPoint.x},${neighborPoint.y}`]
      ) {
        continue;
      }
      ...
      fScore[`${neighborPoint.x},${neighborPoint.y}`] =
        tentativeGScore + heuristic(neighborPoint, goal);
    }
  }
